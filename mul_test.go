...
func tbiMul(x, y Edge) Node {

	node := MakeNode("tbi", nil, []*Edge{&x, &y}, nil, nil)
	node.RunFunc = tbiMulRun
	return node
}

func tbiMulRun (node *Node) {
	x := node.Dsts[0]
	y := node.Dsts[1]
	...
	x.Aux = "Can you multiply an int to a string?"
	y.Aux = int8(77)
	i = 0
	for  {
		if (i > 0) { break }
